...
export default function KnowledgeGarden() {
  ...
  return (
    <section ref={ref} id="articles" className="scroll-mt-28 mb-28 max-w-[50rem] w-full px-4">
      <SectionHeading>
        KNOWLEDGE GARDEN // LEGAL & TECH BRIEFS
      </SectionHeading>

      {/* Filters & Search toolbar */}
      <div className="p-4 rounded-xl border mb-8 flex flex-col md:flex-row gap-4 items-center justify-between bg-[#ebdcb9]/40 border-[#cbd2c0] dark:bg-[#2d251e]/40 dark:border-[#3a2f26] font-mono text-xs">
        {/* Category Tabs */}
        ...
        <div className="relative w-full md:w-64">
          <FiSearch className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#7c6344] dark:text-[#a0896d]" />
          <input
            type="text"
            placeholder="SEARCH CASES/REPORTS..."
            className="w-full pl-9 pr-4 py-1.5 rounded-full text-xs outline-none transition border bg-transparent border-[#c0b090] placeholder-[#8a7a60] focus:border-[#dc2626] text-zinc-950 dark:text-white"
            value={searchQuery}
        ...
        {filteredArticles.length === 0 ? (
          ...
          filteredArticles.map((art) => (
            <motion.article
              ...
              className="p-6 rounded-xl border cursor-pointer hover:shadow-md transition duration-300 relative group flex flex-col justify-between bg-[#ebdcb9]/20 hover:bg-[#ebdcb9]/40 border-[#cbd2c0]/60 dark:border-[#3a2f26]/60 font-mono text-xs"
            ...
            >
              <div>
                <div className="flex items-center gap-4 text-[10px] text-zinc-400 dark:text-zinc-500 mb-2">
                  <span className="px-2 py-0.5 rounded font-bold uppercase bg-[#ebdcb9] text-[#7c6344] dark:bg-[#3e342a] dark:text-[#a0896d]">
                    {art.category}
                  </span>
                ...
                <h3 className="text-base sm:text-lg font-bold group-hover:text-amber-500 transition-colors mb-2 uppercase text-zinc-950 dark:text-white">
                  {art.title}
                </h3>
      ...
      <AnimatePresence>
        {activeArticle && (
          <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 overflow-y-auto">
            ...
            {/* Reader Card */}
            <motion.div
              className="relative w-full max-w-2xl rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh] border transition-all bg-[#fdfbf7] dark:bg-[#1e1b19] border-[#c0b090] dark:border-[#4d4033] font-mono text-xs text-zinc-950 dark:text-[#ebdcd0]"
            ...
            >
              {/* Header bar */}
              <div className="flex items-center justify-between px-6 py-4 border-b shrink-0 bg-[#f5efe2] dark:bg-[#251f19] border-[#cbd2c0] dark:border-[#3a2f26]">
                ...
                <button
                  ...
                  className="p-1 rounded transition hover:bg-[#ebdcb9] text-[#7c6344] dark:hover:bg-[#3e342a] dark:text-[#a0896d]"
                >
              ...
              <div className="flex-1 overflow-y-auto p-6 sm:p-10 legal-pleading">
                <h1 className="text-xl sm:text-2xl font-black mb-6 leading-tight uppercase text-zinc-950 dark:text-white">
                  {activeArticle.title}
                </h1>
                ...
                {/* Paragraph segments */}
                <div className="space-y-6 font-mono text-zinc-800 dark:text-zinc-350">
